...
import { Search, ArrowLeft, Save, Loader2, Plus } from "lucide-react"; // ✅ 清理冗余图标
  ...
  const saveChanges = async () => {
    ...
    try {
      const { error: gError } = await supabase.from('groups').upsert(
        groups.map(g => ({ id: g.id, name: g.name, leader_id: g.leader_id, user_id: user.id }))
      );
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-10 font-sans">
      <header className="max-w-[1440px] mx-auto flex justify-between items-center mb-10">
        <Link href="/dashboard" className="text-slate-500 font-bold flex items-center gap-2 hover:text-slate-800"><ArrowLeft size={20} /> 返回仪表盘</Link>
        ...
        <button onClick={saveChanges} disabled={saving} className="bg-slate-900 text-white px-8 py-3.5 rounded-2xl font-bold flex items-center gap-2 shadow-lg">
          {saving ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />} {saving ? "正在保存..." : "确认保存更改"}
        </button>
      </header>
      ...
      <div className="max-w-[1440px] mx-auto grid grid-cols-[380px_1fr] gap-10">
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm flex flex-col h-[calc(100vh-250px)] overflow-hidden">
          <div className="p-8 border-b border-slate-50">
            <h3 className="text-lg font-black mb-4">待分配名单 ({unassignedStudents.length})</h3>
            <div className="relative">
              <Search size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
              <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="搜索姓名..." className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-50 border-none outline-none" />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto p-6">
            {unassignedStudents.filter(s => s.name.includes(searchTerm)).map(s => (
              <div key={s.id} draggable onDragStart={(e) => e.dataTransfer.setData("sid", s.id)} className="bg-slate-50 p-5 rounded-2xl mb-3 cursor-grab hover:shadow-md transition-shadow">
        ...
        <div className="grid grid-cols-2 gap-8 auto-rows-min">
          {groups.map(g => (
            <div key={g.id} onDragOver={e => e.preventDefault()} onDrop={e => onDrop(e, g.id)} className="bg-white rounded-[32px] border-2 border-dashed border-slate-200 p-8 min-h-[300px]">
              <input value={g.name} onChange={e => setGroups(prev => prev.map(item => item.id === g.id ? { ...item, name: e.target.value } : item))} className="text-xl font-black bg-transparent border-none outline-none mb-6 w-full" />
              <div className="flex flex-col gap-3">
                {g.members.map(m => (
                  <div key={m.id} className="bg-slate-50 p-4 rounded-xl flex justify-between items-center group">
                    <span className="font-bold">{m.name}</span>
                    <button onClick={() => setGroups(prev => prev.map(item => item.id === g.id ? { ...item, leader_id: item.leader_id === m.id ? null : m.id } : item))} className={`px-3 py-1.5 rounded-lg text-xs font-black transition-colors ${g.leader_id === m.id ? 'bg-orange-500 text-white' : 'bg-slate-200 text-slate-500 hover:bg-slate-300'}`}>队长</button>
                  </div>
                ))}
              </div>
            </div>
